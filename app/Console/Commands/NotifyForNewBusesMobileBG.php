...
class NotifyForNewBusesMobileBG extends Command
{
    use Lockable;
     ...
     * @var string
     */
    protected $signature = 'notifier:MobileBGBuses {cssPath?}';
     ...
     * @var string
     */
    protected $description = 'Sends the users the new buses in their specified filters';
     ...
     * @var string $lockKey
     */
    private static $lockKey = 'GettingNewBikes_MobileBG';
    ...
    public function handle()
    {
        ...
        $users->each(function(User $user) {
            ...
            $user->filters()->where("type", "=", "MobileBGBuses")->each(function (Filter $filter) use ($user, $retriever, $collection) {
    ...
    private function sendEmailWithCurrentNewCars(User $user) {
        if ($this->newCars->isNotEmpty()) {
            ...
            $newCarsMail = new NewCars("MobileBG Buses", $this->newCars, $cssPath);
