...
class NotifyForNewBikesMobileBG extends Command
{
    use Lockable;
     ...
     * @var string
     */
    protected $signature = 'notifier:MobileBGBikes {cssPath?}';
    ...
    public function handle()
    {
        ...
        $users->each(function(User $user) {
            ...
            $user->filters()->where("type", "=", "MobileBGBikes")->each(function (Filter $filter) use ($user, $retriever, $collection) {
    ...
    private function sendEmailWithCurrentNewCars(User $user) {
        if ($this->newCars->isNotEmpty()) {
            ...
            $newCarsMail = new NewCars("MobileBG Bikes", $this->newCars, $cssPath);
